...
class AnchorPoints(nn.Module):
    ...
    def forward(self, image):
        ...
        for idx, p in enumerate(self.pyramid_levels):
            ...
            shifted_anchor_points = self.shift(
                image_shapes[idx], self.strides[idx], archor_points
            )
...
class P2PNet(nn.Module):
    def __init__(self):
        ...
        # vgg16 without fc
        self.vgg16_body12 = make_layers(
            [64, 64, "M", 128, 128, "M", 256, 256, 256], in_channels=3, batch_norm=True
        )
        self.vgg16_body3 = make_layers(
            ["M", 512, 512, 512], in_channels=256, batch_norm=True
        )
        self.vgg16_body4 = make_layers(
            ["M", 512, 512, 512], in_channels=512, batch_norm=True
        )
        ...
        self.regression = nn.Sequential(
            *list(
                chain(
                    *[
                        make_conv2d(256, 256, k_size=3, stride=1, padding=1)
                        for _ in range(2)
                    ]
                )
            )
            + make_conv2d(256, 4 * 2, k_size=3, stride=1, padding=1)
        )
        # archor
        ...
        self.classifier = nn.Sequential(
            *list(
            ...
            + make_conv2d(
                256, 4 * 2, k_size=3, stride=1, padding=1, activation="sigmoid"
            )
        )
    ...
    def forward(self, x):
        ...
        regression = (
            regression.permute(0, 2, 3, 1).contiguous().view(regression.shape[0], -1, 2)
        )  # [batch_size, points_num, (x,y)]
        anchor_points = self.anchor_points(x_raw).repeat(
            P4_X.shape[0], 1, 1
        )  # 点的分数? 形状与regression相同 todo: P4_X可否换成x
        ...
        classification = (
            classification.view(batch_size, width, height, 4, 2)
            .contiguous()
            .view(P4_X.shape[0], -1, 2)
        )
...
class P2P_Loss(nn.Module):
    ...
    def loss_points(self, outputs, targets, indices, num_points):
        ...
        target_points = torch.cat(
            [t[i] for t, (_, i) in zip(targets["points"], indices)], dim=0
        )
