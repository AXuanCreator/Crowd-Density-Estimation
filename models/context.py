...
class CANContext(nn.Module):
    def __init__(self, feat_channels, out_channels):
        ...
        self.output_layer = nn.Sequential(
            nn.Conv2d(feat_channels * 2, out_channels, kernel_size=1), nn.ReLU()
        )

    def forward(self, feat: Tensor):
        feat_context = [
            F.interpolate(s_mod(feat), size=feat.shape[2:4], mode="bilinear")
            for s_mod in self.scales_module
        ]
        feat_weight = [self.weight_net(feat - f_c) for f_c in feat_context]

        features = [
            sum(feat_context[i] * feat_weight[i] for i in range(len(self.scales)))
            / sum(feat_weight)
        ] + [feat]
