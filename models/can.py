...
class CANet(nn.Module):
    def __init__(self, use_ckpt=False):
        ...
        self.context = CANContext(
            feat_channels=self.frontend_layer[-1], out_channels=self.backend_layer[0]
        ).to("cuda")
        self.backend = make_layers(
            self.backend_layer,
            in_channels=self.backend_layer[0],
            batch_norm=True,
            dilation=True,
        )
    ...
    def __weight_init(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.normal_(
                    m.weight, std=0.01
                )  # mark: 这里不可以使用kaiming_normal_，会导致梯度爆炸
...
class CanAlexNet(nn.Module):
    def __init__(self, use_ckpt=False):
        ...
        self.upsample = nn.Sequential(
            ...
            nn.ConvTranspose2d(
                in_channels=256, out_channels=256, kernel_size=6, stride=2
            ),
        )

        self.context = CANContext(feat_channels=256, out_channels=256).to("cuda")
        self.backend = make_layers(
            layer=[256, 256, 256, 128, 64, 32],
            in_channels=256,
            batch_norm=True,
            dilation=True,
        )
