...
def main(mode, net):
    ...
    for epoch in tqdm(range(start_epoch, cfg.TRAIN.EPOCHS + 1)):
        ...
        if epoch % cfg.DATA.CKPT_SAVE_EPOCH == 0:
            ...
            save_model(
                path=cfg.DATA.CKPT_SAVE_PATH + f"/{cfg.DATE_TIME}/{net}",
                model=model,
                name=save_name,
            )
...
def train(model, net, criterion, optimizer, epoch):
    ...
    train_dataset: Any = DataLoader(
        CustomDataset(
            root=os.path.join(cfg.DATA.ROOT, "train_data"),
            mode="train",
            net=net,
    ...
    for i, data in enumerate(train_dataset):
        ...
        gt = (
            {k: v.to("cuda", dtype=torch.float32) for k, v in data["gt"].items()}
            if isinstance(data["gt"], dict)
            else data["gt"].to("cuda", dtype=torch.float32)
        )
        ...
        if i % cfg.TRAIN.LOG == 0:
            pred_cnt = (
                int(
                    (
                        torch.nn.functional.softmax(output["pred_logits"], -1)[:, :, 1][
                            0
                        ]
                        > 0.5
                    ).sum()
                ...
                if net == "p2p"
                else torch.sum(output).item()
            )
            tgt_cnt = (
                torch.sum(gt["labels"]).item() if net == "p2p" else torch.sum(gt).item()
            )
            print(
                f"epoch:[{epoch}][{i}/{len(train_dataset)}]\tloss:{loss.item():.3f}->avg:{total_loss / (i + 1):.3f}\tpred:{pred_cnt:.3f}->tgt:{tgt_cnt:3f}"
            )
...
def valid(model, net, epoch):
    ...
    valid_dataset: Any = DataLoader(
        CustomDataset(
            root=os.path.join(cfg.DATA.ROOT, "test_data"),
            mode="valid",
            net=net,
            transform=cfg.DATA.TRANSFORM,
        ),
    ...
    for i, data in enumerate(valid_dataset):
        ...
        if net in ["can", "can-alex"]:
            pred_cnt = sum(
                item.squeeze().detach().cpu().numpy().sum() for item in output_list
            )
            tgt_cnt = torch.sum(gt).item()
        elif net == "p2p":
            outputs_scores_list = [
                torch.nn.functional.softmax(d["pred_logits"], -1)[:, :, 1][0]
                for d in output_list
            ]
            pred_index_list = [
                (scores.detach().cpu().numpy() > 0.5) for scores in outputs_scores_list
            ]
        ...
        if epoch % cfg.DATA.SAVE_IMAGE_EPOCH == 0 and random.random() > 0.5 or True:
            ...
            if net in ["can", "can-alex"]:
                merge = merge_density(
                    [
                        density.squeeze().detach().cpu().numpy()
                        for density in output_list
                    ]
                )
                ...
                for idx, (pred_index, output) in enumerate(
                    zip(pred_index_list, output_list)
                ):
                    pred_coord = (
                        output["pred_points"]
                        .squeeze()
                    ...
                    pred_coord_list.append(
                        pred_coord
                        + [0 if idx < 2 else w_biset, 0 if idx % 2 == 0 else h_biset]
                    )  # 将1/4坐标映射到全局坐标
                pred_coord = np.concatenate(pred_coord_list, axis=0).astype(
                    int
                )  # 合并坐标数组
                plot_points_on_rgb(
                    coords=pred_coord,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode", "-m", type=str, default="valid", help="train/valid/both"
    )
    parser.add_argument(
        "--net", "-n", type=str, default="can-alex", help="models: can/can-alex/p2p"
    )
