...
MODEL_MAP = {"can": CANet, "can-alex": CanAlexNet, "p2p": P2PNet}
...
def generate_h5(gt_path, h, w, net=None):
    ...
    for gmp in gt_mat_path:
        ...
        if net != "p2p":
            k = gaussian_filter(
                k, 15
            )  # 高斯滤波会让值"散开"，但总值(sum)几乎不变。此举是为了提高神经网络的的容错率，加快收敛
...
def save_density_image(x, path=None, binarization=True, up_sample=None):
    ...
    if up_sample is not None:
        x = cv2.resize(
            x, dsize=None, fx=up_sample, fy=up_sample, interpolation=cv2.INTER_LANCZOS4
        )
...
def save_image_with_contours(binary_x, rgb, path=None):
    ...
    todo: 优化检测框，二值化太极端了
    """
    assert len(binary_x.shape) == 2 and len(rgb.shape) == 3, (
        "binary_x shape: [h, w] | rgb shape: [h, w, 3]"
    )
