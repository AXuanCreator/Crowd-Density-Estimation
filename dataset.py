...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # rgb&gt图输出
        if cfg.DEBUG:
            combine_image = np.hstack(
                (image, np.repeat(np.expand_dims(gt, axis=-1), 3, axis=-1) * 255)
            )
        ...
        image = (
            ...
            if self.transform
            else transforms.ToTensor()(image.copy()) / 255
        )
        ...
        if self.net in ["can", "can-alex"]:
            ...
            batch = {
                "image": image,
                "gt": {"labels": labels, "points": points},
                "info": info,
                "rgb": rgb,
            }
        else:
    ...
    def _prepare_gt(self):
        if self.net in ["can", "can-alex"]:
            if len(glob.glob(self.gt_path + "/*can*.h5")) != len(self.image_list):
                ...
                generate_h5(
                    self.gt_path, *cv2.imread(self.image_list[0]).shape[:2], net="can"
                )
            ...
            if len(glob.glob(self.gt_path + "/*p2p*.h5")) != len(self.image_list):
                ...
                generate_h5(
                    self.gt_path, *cv2.imread(self.image_list[0]).shape[:2], net="p2p"
                )
    ...
    def _data_enhancement(self, image, gt):
        ...
        if self.scaling is not None and self.net != "p2p":
            crop_gt = (
                cv2.resize(
                    crop_gt,
                    (int(crop_gt.shape[1] / 8), int(crop_gt.shape[0] / 8)),
    ...
    def _data_downsample(self, image, gt, downsample=4):
        image = cv2.resize(
            image,
            (image.shape[1] // downsample, image.shape[0] // downsample),
            interpolation=cv2.INTER_CUBIC,
        )
